...
# def read_exr_disp(filepath: str) -> np.ndarray:
#     """Read disparity map from .exr file.
#     Args:
...
#     # parse the data type from the header
#     data_type = Imath.PixelType(Imath.PixelType.FLOAT)

#     # parse the size from the header
#     data_window = file.header()["dataWindow"]
#     max_x, min_x = data_window.max.x, data_window.min.x
#     max_y, min_y = data_window.max.y, data_window.min.y
#     size = (max_x - min_x + 1, max_y - min_y + 1)

#     disp_data = np.fromstring(file.channel("G", data_type), dtype=np.float32)
#     disp = disp_data.reshape(size[1], size[0])
