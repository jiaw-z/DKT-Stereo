...
class SparseFlowAugmentor_RTClean:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=False, yjitter=False, saturation_range=[0.7,1.3], gamma=[1,1,1,1]):
        # spatial augmentation params
        self.crop_size = crop_size
        self.min_scale = min_scale
        ...
        # photometric augmentation params
        self.photo_aug = Compose([ColorJitter(brightness=0.3, contrast=0.3, saturation=saturation_range, hue=0.3/3.14), AdjustGamma(*gamma)])
        self.asymmetric_color_aug_prob = 1.0
        self.eraser_aug_prob = 0.5
    ...
    def spatial_transform(self, img1_clean, img2_clean, img1, img2, flow, valid):
        # randomly sample scale
        ...
        return img1_clean, img2_clean, img1, img2, flow, valid


    def __call__(self, img1, img2, flow, valid):
        img1_clean = np.array(img1)
        img2_clean = np.array(img2)
        # img1_clean, img2_clean, img1, img2, flow, valid = self.spatial_transform(img1_clean, img2_clean, img1, img2, flow, valid)
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1_clean, img2_clean, img1, img2, flow, valid = self.spatial_transform(img1_clean, img2_clean, img1, img2, flow, valid)
