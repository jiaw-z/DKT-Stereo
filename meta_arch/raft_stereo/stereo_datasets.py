...
class ETH3D(StereoDataset):
    def __init__(self, aug_params=None, root='datasets/ETH3D', split='training'):
        ...
        disp_list = sorted( glob(osp.join(root, 'two_view_training/*/disp0GT.pfm')) ) if split == 'training' else [osp.join(root, 'two_view_training/playground_1l/disp0GT.pfm')]*len(image1_list)
...
class KITTI2012(StereoDataset):
    def __init__(self, aug_params=None, root='datasets/KITTI2012', image_set='training'):
        super(KITTI2012, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI)
        assert os.path.exists(root)
...
class Booster(StereoDataset):
    def __init__(self, aug_params=None, root='datasets/booster', resolution='Q', split='train'):
        super(Booster, self).__init__(aug_params, sparse=True)
        assert resolution == 'Q'
        if resolution == 'F':
            root = root + '/booster_gt'
        elif resolution == 'Q':
            root = root + '/Booster_Quarter'
