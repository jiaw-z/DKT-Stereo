...
class CorrBlock1D_Cosine:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4, mix=False):
        self.num_levels = num_levels
        self.radius = radius
        self.corr_pyramid = []
        ...
        # all pairs correlation
        if mix:
            corr = CorrBlock1D_Cosine.corr_mix(fmap1, fmap2)
        else:
            corr = CorrBlock1D_Cosine.corr(fmap1, fmap2)
    ...
    def corr(fmap1, fmap2):
        ...
        fmap1 = fmap1.view(B, D, H, W1)
        fmap2 = fmap2.view(B, D, H, W2)
        fmap1 = fmap1 / fmap1.norm(dim=1, keepdim=True)
        fmap2 = fmap2 / fmap2.norm(dim=1, keepdim=True)
        corr = torch.einsum('aijk,aijh->ajkh', fmap1, fmap2)
        corr = corr.reshape(B, H, W1, 1, W2).contiguous()
        assert (corr.min() >= -1.00001) and (corr.max() <= 1.00001), [corr.min(), corr.max()]
        return corr

    @staticmethod
    def corr_mix(fmap1, fmap2):
        corr_list = []
        for i in range(len(fmap1)):
            feat1 = fmap1[i]
            feat2 = fmap2[i]
            corri = CorrBlock1D_Cosine.corr(feat1, feat2)
            corr_list.append(corri)
        assert len(corr_list) == 2
        prob = random.random()
        assert prob >=0 and prob <=1, [prob]
        corr0 = corr_list[0]
        corr1 = corr_list[1]
        corr = corr0 * prob + corr1 * (1-prob)
        return corr
