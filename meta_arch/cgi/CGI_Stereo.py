...
class CGI_Stereo(nn.Module):
    ...
    def forward(self, left, right, iters=-1, flow_init=None, test_mode=False):
        left = normalize_imagenet(left)
        right = normalize_imagenet(right)
        ...
        pred_up = context_upsample(pred, spx_pred).unsqueeze(1)


        if self.training:
            results = {}
            results.update(disp_preds=[-pred*4, -pred_up*4])
            return results

        else:
            return None, -pred_up*4
