...
from .extractor import MultiBasicEncoder, ResidualBlock
...
class PCVNet(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, test_mode=False, init_param=None, cascade=False):
        ...
        if cascade:
            ...
            if test_mode:
                return None, refined_disp_up
            results = {}
            results.update(disp_preds=[refined_disp_up])
            results.update(output_list=[refined_disp_up, disp_seqs, mu_seqs, w_seqs, sigma_seqs])
            return results
